 ...
 */
@WebServlet(urlPatterns = { "/FeedbackStatus" })
public class FeedbackStatus extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			if (con != null)
				System.out.println("FeedbackStatus-connection established");
		} catch (Exception e) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			try {
				...
				int r1 = stat.executeUpdate("update feedback set reply_status=\"REPLIED\" where id =" + id + ";");
				if (r1 > 0) {
					String path = "ViewFeedback.jsp?param=changed";
					response.sendRedirect(path);
				} else {
					String path = "ViewFeedback.jsp?param=not_changed";
					response.sendRedirect(path);
				}
			} catch (Exception e) {
				e.printStackTrace();
