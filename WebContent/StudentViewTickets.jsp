...
String email_id=(String)session.getAttribute("semail");
String firstName=(String)session.getAttribute("sfirstname");
Statement stat=con.createStatement();
ResultSet r=stat.executeQuery("select ticket_id,email_id,name,college,university,level,keyword,subject,urgency,description,anonymity,ticket_date,ticket_time,status from ticket where email_id='"+email_id+"';");
int opened_count=0,progress_count=0,closed_count=0;
Statement stat1=con.createStatement();
Statement stat2=con.createStatement();
Statement stat3=con.createStatement();

ResultSet rs1 = stat1.executeQuery("select count(*) as 'count' from ticket where email_id='"+email_id+"' and status = 'OPENED';");
while(rs1.next()){
	opened_count = rs1.getInt("count");
}
ResultSet rs2 = stat2.executeQuery("select count(*) as 'count' from ticket where email_id='"+email_id+"' and status = 'IN PROGRESS';");
while(rs2.next()){
    progress_count = rs2.getInt("count");
}
ResultSet rs3 = stat3.executeQuery("select count(*) as 'count' from ticket where email_id='"+email_id+"' and status = 'CLOSED';");
while(rs3.next()){
...
<body>
	...
	<div class="container" style="display:flex;justify-content:center">
		<div>
			<button onclick="func1();" class="btn btn-primary opened_button text-white active"><b
					style="font-size:1vw;">OPENED<br><%out.println(opened_count);%></b></button>
		</div>&nbsp;&nbsp;&nbsp;&nbsp;
		<div>
			<button onclick="func2();" class="btn btn-success inProgress_button text-white active"><b
					style="font-size:1vw;">IN PROGRESS<br><%out.println(progress_count);%></b></button>
		</div>&nbsp;&nbsp;&nbsp;&nbsp;
		<div>
			<button onclick="func3();" class="btn btn-danger closed_button text-white active"><b
					style="font-size:1vw;">CLOSED<br><%out.println(closed_count);%></b></button>
		</div>
	</div>
	<br>
    ...
    &nbsp;&nbsp;
   	...
   	<!-- FILTER SECTION END-->   
	<div class="container-fluid bg-light table-responsive-md table-responsive-xs">
...
while(r.next())
	...
	String status=r.getString("status");
		...
		out.println("<td><a target=\"_blank\" href=ViewTicketDetails.jsp?id="+r.getInt("ticket_id")+">"+r.getInt("ticket_id")+"</a></td><td>"+r.getString("level")+"</td><td>"+r.getString("keyword")+"</td><td>"+r.getString("subject")+"</td><td>"+r.getString("urgency")+"</td><td>"+r.getDate("ticket_date")+"</td><td>"+r.getTime("ticket_time")+"</td><td>"+"<a class=\"btn btn-success\" role=\"button\" href=StudentEditTicket.jsp?id="+r.getInt("ticket_id")+">EDIT</a>&nbsp"+
				"<a class=\"btn btn-danger\" role=\"button\" onclick=\"return confirm('Are you sure?')\" href=DeleteTicket?id="+r.getInt("ticket_id")+">DELETE</a></td>");
		}
		else if(status.equals("IN PROGRESS"))
		{
			out.println("<tr class=\"i\">");
			out.println("<td><a target=\"_blank\" href=ViewTicketDetails.jsp?id="+r.getInt("ticket_id")+">"+r.getInt("ticket_id")+"</a></td><td>"+r.getString("level")+"</td><td>"+r.getString("keyword")+"</td><td>"+r.getString("subject")+"</td><td>"+r.getString("urgency")+"</td><td>"+r.getDate("ticket_date")+"</td><td>"+r.getTime("ticket_time")+"</td><td>"+"<button type=\"button\" class=\"btn btn-success\" disabled>IN PROGRESS</button></td>");
		}
		else
		{
			out.println("<tr class=\"c\">");
			out.println("<td><a target=\"_blank\" href=ViewTicketDetails.jsp?id="+r.getInt("ticket_id")+">"+r.getInt("ticket_id")+"</a></td><td>"+r.getString("level")+"</td><td>"+r.getString("keyword")+"</td><td>"+r.getString("subject")+"</td><td>"+r.getString("urgency")+"</td><td>"+r.getDate("ticket_date")+"</td><td>"+r.getTime("ticket_time")+"</td><td>"+"<a class=\"btn btn-success\" role=\"button\" onclick=\"return confirm('Are you sure?')\" href=ChangeStatus?id="+r.getInt("ticket_id")+"&status=reopen>RE-OPEN</a></td>");
		}
		out.println("</tr>");
	} 
} 
...
<script type="text/javascript">
	...
	if (url == "param=deleted") {
		$('.hidden').addClass('alert alert-success');
		$('.hidden').show();
		document.getElementById('gg').innerHTML = "SUCCESSFULLY DELETED TICKET";
	}
	if (url == "param=cancel") {
		$('.hidden').addClass('alert alert-success');
		$('.hidden').hide();
	...
	if (url == "param=edited") {
		$('.hidden').addClass('alert alert-success');
		$('.hidden').show();
		document.getElementById('gg').innerHTML = "SUCCESSFULLY EDITED TICKET DETAILS";
	}
	if (url == "param=wrong") {
		$('.hidden').addClass('alert alert-success');
		$('.hidden').show();
