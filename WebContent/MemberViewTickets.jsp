...
<style>
.opened_button, .closed_button, .inProgress_button {
...
System.out.println("ViewTicketsMember-Connection Established");
...
String college=(String)session.getAttribute("mcollege");
String university=(String)session.getAttribute("muniversity");		
int opened_count=0,progress_count=0,closed_count=0;
Statement stat=con.createStatement();
Statement stat1=con.createStatement();
Statement stat2=con.createStatement();
Statement stat3=con.createStatement();
ResultSet r,rs1,rs2,rs3;
...
"description,anonymity,ticket_date,ticket_time,status from ticket where level='"+jurisdiction+"' and college='"+college+"';");
rs1 = stat1.executeQuery("select count(*) as 'count' from ticket where level='"+jurisdiction+"' and college='"+college+"' and status = 'OPENED';");
while(rs1.next()){
opened_count = rs1.getInt("count");
}
rs2 = stat2.executeQuery("select count(*) as 'count' from ticket where level='"+jurisdiction+"' and college ='"+college+"' and status = 'IN PROGRESS';");
while(rs2.next()){
progress_count = rs2.getInt("count");
}
rs3 = stat3.executeQuery("select count(*) as 'count' from ticket where level='"+jurisdiction+"' and college = '"+college+"' and status = 'CLOSED';");
while(rs3.next()){
...
{
r=stat.executeQuery("select ticket_id,email_id,name,college,university,level,keyword,subject,urgency,"+
"description,anonymity,ticket_date,ticket_time,status from ticket where level='"+jurisdiction+"' and university='"+university+"';");				
rs1 = stat1.executeQuery("select count(*) as 'count' from ticket where level='"+jurisdiction+"' and university='"+university+"' and status = 'OPENED';");
while(rs1.next()){
opened_count = rs1.getInt("count");
}
rs2 = stat2.executeQuery("select count(*) as 'count' from ticket where level='"+jurisdiction+"' and university='"+university+"' and status = 'IN PROGRESS';");
while(rs2.next()){
progress_count = rs2.getInt("count");
}
rs3 = stat3.executeQuery("select count(*) as 'count' from ticket where level='"+jurisdiction+"' and university='"+university+"' and status = 'CLOSED';");
while(rs3.next()){
closed_count = rs3.getInt("count");
}
}
...
<div>  
<button onclick="func1();" class="btn btn-primary text-white opened_button active"><b>OPENED<br><%out.println(opened_count);%></b></button>  
</div>&nbsp;&nbsp;&nbsp;&nbsp;  
<div>  
<button onclick="func2();" class="btn btn-success text-white inProgress_button active "><b>IN PROGRESS<br><%out.println(progress_count);%></b></button>  
</div>&nbsp;&nbsp;&nbsp;&nbsp;  
<div>  
<button onclick="func3();" class="btn btn-danger text-white closed_button active"><b>CLOSED<br><%out.println(closed_count);%></b></button>  
</div>  
</div>  
<br>
<!-- FILTER SECTION -->
    ...
    &nbsp;&nbsp;
   	...
   	<!-- FILTER SECTION END-->
   	<div class="container-fluid table-responsive-md table-responsive-xs">   
...
out.println("<td><a target=\"_blank\" href=ViewTicketDetails.jsp?id="+r.getInt("ticket_id")+">"+r.getInt("ticket_id")+"</a></td><td>"+r.getString("keyword")+"</td><td>"+r.getString("subject")+"</td><td>"+r.getString("urgency")+"</td><td>"+r.getDate("ticket_date")+"</td><td>"+r.getTime("ticket_time")+"</td><td style=\"text-align:center\">"+"<a class=\"btn btn-success\" role=\"button\" onclick=\"return confirm('Are you sure?')\" href=ChangeStatus?id="+r.getInt("ticket_id")+"&status=progress>MOVE TO PROGRESS</a></td>");
}
else if(status.equals("IN PROGRESS"))
{
out.println("<tr class=\"i\">");
out.println("<td><a target=\"_blank\" href=ViewTicketDetails.jsp?id="+r.getInt("ticket_id")+">"+r.getInt("ticket_id")+"</a></td><td>"+r.getString("keyword")+"</td><td>"+r.getString("subject")+"</td><td>"+r.getString("urgency")+"</td><td>"+r.getDate("ticket_date")+"</td><td>"+r.getTime("ticket_time")+"</td><td style=\"text-align:center\">"+"<a class=\"btn btn-success\" role=\"button\" onclick=\"return confirm('Are you sure?')\" href=ChangeStatus?id="+r.getInt("ticket_id")+"&status=close>CLOSE</a></td>");
}
else
{
out.println("<tr class=\"c\">");
out.println("<td><a target=\"_blank\" href=ViewTicketDetails.jsp?id="+r.getInt("ticket_id")+">"+r.getInt("ticket_id")+"</a></td><td>"+r.getString("keyword")+"</td><td>"+r.getString("subject")+"</td><td>"+r.getString("urgency")+"</td><td>"+r.getDate("ticket_date")+"</td><td>"+r.getTime("ticket_time")+"</td><td style=\"text-align:center\">"+"<button type=\"button\" class=\"btn btn-secondary\" disabled>CLOSED</button></td>");
}
out.println("</tr>");
}
} 
